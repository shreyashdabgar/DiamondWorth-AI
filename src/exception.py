...
def why_error(error, error_details:sys):
    ...
    file_name = exc_tb.tb_frame.f_code.co_filename # for findin file name which file we getv error from
    error_details = ("error in file : ",file_name,"at line number :", exc_tb.tb_lineno, "and the error is :", str(error))
    return error_details

class coustemexception(Exception):
    def __init__(self, error, error_details:sys):# we are taking error and error_details from def error and put into init class becuase init is package which is used as package and import everywhere
